(function(a){(a.homer=a.homer||{}).modules={app:{name:"homerApp"}}})(window);function isUndefined(a){return"undefined"==typeof a}function defineHomerAngularModule(a,b){return angular.module(a,b)}function defineHomerJsLibrary(a,b,d){if(isUndefined(d)&&(d=b,b=[],d instanceof Array))for(var c=0;c<d.length-1;++c)b.push(d[c]);return angular.module(a,b).factory(a,d)};(function(a,b){b.modules.core={name:"homer-core",services:{eventbus:"eventbus",search:"search",profile:"userProfile",store:"storeService"}};a.module(b.modules.core.name,["ui.router"])})(angular,homer);(function(a,b){a.module(b.modules.core.name).factory(b.modules.core.services.profile,["$q","$http","eventbus","$log",function(b,c,e,g){var h=!1,f={},l={timerange:{from:new Date((new Date).getTime()-9E5),to:new Date},search:{},transaction:{},result:{},node:{},limit:200};f.result=l.result;f.node=l.node;f.transaction=l.transaction;f.limit=l.limit;f.timerange=l.timerange;f.search=l.search;var k=function(a){return a.lastName+a.firstName},m=function(a){delete f[a]},n=function(a,f){var e="api/profile/store";
null!=a&&(e="api/profile/store/"+a);var g={id:a,param:f},l=b.defer();c.post(e,g).then(function(a){l.resolve(a.data)},function(a){l.reject()});return l.promise},q=function(a,f){var e=b.defer();c["delete"]("api/profile/store/"+a).then(function(a){e.resolve(a.data)},function(a){e.reject()});return e.promise};return{setProfile:function(a,b){f[a]=b;n(a,b)},getProfile:function(a){return f[a]},deleteProfile:function(a){m(a);q(a)},setLocalProfile:function(a,b){f[a]=b},getLocalProfile:function(a){return f[a]},
deleteLocalProfile:m,getRemoteProfile:function(a){var e=b.defer();c.get("api/profile/store/"+a).success(function(a){var b=JSON.parse(value);"timerange"==k&&(b.from=new Date(b.from),b.to=new Date(b.to),l.timerange=b);f[k]=b;l[k]=b;e.resolve(a.data)}).error(function(){e.reject()});return e.promise},setRemoteProfile:n,deleteRemoteProfile:q,getAll:function(){var e=b.defer();if(1==h)return e.resolve(h),e.promise;c.get("api/profile/store").success(function(b){a.forEach(b.data,function(b,d){var c=a.fromJson(b);
"timerange"==d&&(c.from=new Date(c.from),c.to=new Date(c.to),l.timerange=c);f[d]=c;l[d]=c;h=!0;e.resolve(d)})}).error(function(a){e.reject("Error: request returned status "+a)});return e.promise},profileScope:l}}])})(angular,homer);(function(a,b){a.module(b.modules.core.name).factory(b.modules.core.services.eventbus,["$rootScope",function(a){return{subscribe:function(b,e){return a.$on(b,e)},broadcast:function(b,e){a.$emit(b,e)}}}])})(angular,homer);(function(a,b){a.module(b.modules.core.name).factory(b.modules.core.services.search,["$q","$http","eventbus",function(a,b,e){var g={},h={fromdate:new Date((new Date).getTime()-3E5),todate:new Date},f={timestamp:{from:new Date((new Date).getTime()-3E5),to:new Date},param:{transaction:{call:!0,registration:!0,rest:!0},limit:200}};return{searchByMethod:function(f){var e=a.defer();b.post("api/search/data",f).then(function(a){"false"==a.data.auth?e.reject("user not authorized"):e.resolve(a.data.data)},
function(a){e.reject("bad response combination")});return e.promise},searchMethod:function(f){var e=a.defer();b.post("api/search/method",f).then(function(a){"false"==a.data.auth?e.reject("user not authorized"):e.resolve(a.data.data)},function(a){e.reject("bad response combination")});return e.promise},searchMessage:function(e){var f=a.defer();b.post("api/search/message",e).then(function(a){"false"==a.data.auth?f.reject("user not authorized"):f.resolve(a.data.data)},function(a){f.reject("bad response combination")});
return f.promise},searchTransaction:function(f){var e=a.defer();b.post("api/search/transaction",f).then(function(a){"false"==a.data.auth?e.reject("user not authorized"):e.resolve(a.data.data)},function(a){e.reject("bad response combination")});return e.promise},makePcapTextforTransaction:function(e,f){var g=a.defer(),h="api/search/export/";b.post(1==f?h+"text":h+"pcap",e,{responseType:"arraybuffer"}).then(function(a){"false"==a.data.auth?g.reject("user not authorized"):g.resolve(a.data)},function(a){g.reject("bad response combination")});
return g.promise},makePcapTextData:function(e,f){var g=a.defer(),h="api/search/export/data/";b.post(1==f?h+"text":h+"pcap",e,{responseType:"arraybuffer"}).then(function(a){"false"==a.data.auth?g.reject("user not authorized"):g.resolve(a.data)},function(a){g.reject("bad response combination")});return g.promise},createShareLink:function(e){var f=a.defer();b.post("api/search/sharelink",e).then(function(a){"false"==a.data.auth?f.reject("user not authorized"):f.resolve(a.data.data)},function(a){f.reject("bad response combination")});
return f.promise},getTimeRange:function(){return h},setTimeRange:function(a){h=a},getSearchData:function(){return f},setSearchData:function(a){f=a},getSearchValue:function(){return g},setSearchValue:function(a){g=a},searchValue:g,searchRTCPReport:function(f){var e=a.defer();b.post("api/report/rtcp",f).then(function(a){"false"==a.data.auth?e.reject("user not authorized"):e.resolve(a.data.data)},function(a){e.reject("bad response combination")});return e.promise},searchLogReport:function(e){var f=a.defer();
b.post("api/report/log",e).then(function(a){"false"==a.data.auth?f.reject("user not authorized"):f.resolve(a.data.data)},function(a){f.reject("bad response combination")});return f.promise},searchQualityReport:function(f,e){var g=a.defer();b.post("api/report/quality/"+f,e).then(function(a){"false"==a.data.auth?g.reject("user not authorized"):g.resolve(a.data.data)},function(a){g.reject("bad response combination")});return g.promise},loadNode:function(){var f=a.defer();b.get("api/dashboard/node").then(function(a){"false"==
a.data.auth?f.reject("user not authorized"):f.resolve(a.data.data)},function(a){f.reject("bad response combination")});return f.promise}}}])})(angular,homer);(function(a,b){a.module(b.modules.core.name).factory(b.modules.core.services.store,["$q","$http","eventbus",function(a,b,e){return{getAll:function(){var e=a.defer();b.get("api/dashboard/store").then(function(a){e.resolve(a.data)},function(a){e.reject()});return e.promise},get:function(e){var h=a.defer();b.get("api/dashboard/store/"+e).success(function(a){h.resolve(a.data)}).error(function(){h.reject()});return h.promise},set:function(e,h){var f=a.defer();b.post("api/dashboard/store/"+e,h).then(function(a){f.resolve(a.data)},
function(a){f.reject()});return f.promise},"delete":function(e,h){var f=a.defer();b["delete"]("api/dashboard/store/"+e).then(function(a){f.resolve(a.data)},function(a){f.reject()});return f.promise},menu:function(e,h){var f=a.defer();b.post("api/dashboard/menu/"+e,h).then(function(a){f.resolve(a.data)},function(a){f.reject()});return f.promise}}}])})(angular,homer);(function(a,b){a.module(b.modules.core.name).directive("compareTo",function(){return{scope:{targetModel:"=compareTo"},require:"ngModel",link:function(a,b,e,g){a.$watch(function(){var e=b.val(),f=a.targetModel;return null!==f?e===f:!1},function(a){g.$setValidity("errorCompareTo",a)})}}}).directive("scroll",function(b){return function(c,e,g){a.element(b).bind("scroll",function(){c.boolChangeClass=50<=this.pageYOffset?!0:!1;c.$apply()})}}).directive("autoHeight",["$window","$timeout",function(b,c){return function(c,
...
controller:b.modules.auth.controllers.login,templateUrl:"js/modules/auth/html/not-authorised.tmpl.html"});c.otherwise(b.modules.pages.routes.home)}])})(angular,homer);(function(a,b){a.module(b.modules.auth.name).factory(b.modules.auth.services.authentication,["$q","$http","eventbus",function(a,c,e){var g,h=function(a,b,d){return{isAuthenticated:b,name:a,permissions:d}};return{login:function(f,l){var k=a.defer();c.post("api/session",{username:f,password:l}).then(function(a){console.log(a.data);if("false"==a.data.auth)console.log("AAAA:"+a.data.auth),k.reject("Unknown Username / Password combination");else{console.log("EMAIL 111");var d=a.data.data.grp.split(",");
console.log(d);g=h(f,!0,d);console.log(a.data.data.grp);e.broadcast(b.modules.auth.events.userLoggedIn,g);k.resolve(g)}},function(a){console.log(a);console.log("bad result");k.reject("Unknown Username / Password combination")});return k.promise},logout:function(){var f=a.defer();g=void 0;e.broadcast(b.modules.auth.events.userLoggedOut);c["delete"]("api/session").then(function(a){f.resolve(!1)},function(a){f.reject(!1)});return f.promise},session:function(){var f=a.defer();c.get("api/session").then(function(a){if("false"==
a.data.auth)g=h("",!1,""),e.broadcast(b.modules.auth.events.usernotAuthorised,g),f.reject("Unknown Username / Password combination");else{var d=a.data.data.grp.split(",");g=h(a.data.data.username,!0,d);e.broadcast(b.modules.auth.events.userLoggedIn,g);f.resolve(g)}},function(a){f.reject("Unknown Username / Password combination")});return f.promise},user:function(){var f=a.defer();c.get("api/user").then(function(a){if("false"==a.data.auth)console.log("SESSION:"+a.data.auth),f.reject("unknown username");
else{var d=a.data.data.user.grp.split(",");g=h(a.data.data.user.username,!0,d);e.broadcast(b.modules.auth.events.userLoggedIn,g);f.resolve(g)}},function(a){f.reject("Unknown Username")});return f.promise},getCurrentLoginUser:function(){return g}}}])})(angular,homer);(function(a,b){a.module(b.modules.auth.name).factory(b.modules.auth.services.authorization,["authentication",function(d){return{authorize:function(c,e,g){var h=b.modules.auth.enums.authorised.authorised,f=d.getCurrentLoginUser(),l=[],k=!0;g=g||b.modules.auth.enums.permissionCheckType.atLeastOne;if(!0===c&&void 0===f)h=b.modules.auth.enums.authorised.loginRequired;else if(!0===c&&void 0!==f&&(void 0===e||0===e.length))h=b.modules.auth.enums.authorised.authorised;else if(e){l=[];a.forEach(f.permissions,
function(a){l.push(a.toLowerCase())});for(h=0;h<e.length;h+=1)if(c=e[h].toLowerCase(),g===b.modules.auth.enums.permissionCheckType.combinationRequired){if(k=k&&-1<l.indexOf(c),!1===k)break}else if(g===b.modules.auth.enums.permissionCheckType.atLeastOne&&(k=-1<l.indexOf(c)))break;h=k?b.modules.auth.enums.authorised.authorised:b.modules.auth.enums.authorised.notAuthorised}return h}}}])})(angular,homer);(function(a,b){a.module(b.modules.auth.name).controller(b.modules.auth.controllers.login,["$rootScope","$scope","$location","$modal",b.modules.auth.services.authentication,b.modules.core.services.profile,function(a,c,e,g,h,f){c.loginModel={};c.isBusy=!1;c.invalidLogin=!1;a.loggedIn=!1;c.login=function(){console.log("Submiting user info.");c.invalidLogin=!1;c.isBusy=!0;h.login(c.loginModel.username,c.loginModel.password).then(function(g){c.isBusy=!1;1==g.isAuthenticated?(a.loggedIn=!0,console.log(f),
...
resultSearchSubmit:"resultSearchSubmit",hideLeftMenu:"hideLeftMenu"},state:{home:"home",search:"search",result:"result",dashboard:"dashboard"}};b.modules.pages.instance=a.module(b.modules.pages.name,["ui.router"])})(angular,homer);(function(a,b){b.modules.pages.instance.config(["$stateProvider",function(a){a.state(b.modules.pages.state.home,{url:b.modules.pages.routes.home,controller:b.modules.pages.controllers.dashboard,controllerAs:"dashboard",templateUrl:"js/modules/pages/html/home.tmpl.html"});a.state(b.modules.pages.state.search,{url:b.modules.pages.routes.search,controller:b.modules.pages.controllers.dashboard,controllerAs:"dashboard",templateUrl:"js/modules/pages/html/search.tmpl.html"});a.state(b.modules.pages.state.result,
{url:b.modules.pages.routes.result,controller:b.modules.pages.controllers.result,templateUrl:"js/modules/pages/html/result.tmpl.html",resolve:{user:function(a,b){return b.getAll()}}});a.state(b.modules.pages.state.dashboard,{url:b.modules.pages.routes.dashboard,controller:b.modules.pages.controllers.dashboard,controllerAs:"dashboard",templateUrl:"js/modules/pages/html/dashboard.tmpl.html"})}])})(angular,homer);(function(a,b){a.module(b.modules.pages.name).controller(b.modules.pages.controllers["default"],["$location","$rootScope",b.modules.auth.services.authentication,function(a,c,e){0==c.loggedIn?(console.log("AUTH FALSE"),a.path(b.modules.auth.routes.login)):console.log("AUTH TRUE")}])})(angular,homer);(function(a,b){a.module(b.modules.pages.name).controller(b.modules.pages.controllers.search,["$scope","$rootScope","eventbus","$http","$location",b.modules.core.services.search,b.modules.core.services.store,function(a,c,e,g,h,f,l){a.expandme=!0;a.showtable=!1;0==c.loggedIn?(console.log("AUTH FALSE"),h.path(b.modules.auth.routes.login)):(console.log("AUTH TRUE"),a.colapseForm=function(){a.expandme=!a.expandme},a.removeForm=function(){a.expandme=!1},a.processSearchForm=function(c){a.showtable=!0;a.expandme=
!1;c=f.getTimeRange();c={timestamp:{from:c.fromdate.getTime(),to:c.todate.getTime()},param:{transaction:{call:!0,registration:!0,rest:!0},limit:100}};f.setSearchData(c);f.setSearchValue(f.searchValue);f.searchValue={};l.setProfile(null,[{name:"timestamp",value:c.timestamp},{name:"param",value:c.param}]).then(function(a){});e.broadcast(b.modules.pages.events.resultSearchSubmit,"fullsearch");h.path("/result")})}])})(angular,homer);(function(a,b){a.module(b.modules.pages.name).controller(b.modules.pages.controllers.result,["$scope","$rootScope","eventbus","$http","$location",b.modules.core.services.search,"$timeout","$window","$homerModal",b.modules.core.services.profile,function(d,c,e,g,h,f,l,k,m,n){d.expandme=!0;d.showtable=!0;d.dataLoading=!1;0==c.loggedIn?(console.log("AUTH FALSE"),h.path(b.modules.auth.routes.login)):(console.log("AUTH TRUE"),e.broadcast(b.modules.pages.events.hideLeftMenu,"1"),d.processSearchResult=function(){var b=
{param:{},timestamp:{}},c=n.getProfile("transaction"),e=n.getProfile("limit"),g=n.getProfile("timerange"),h=n.getProfile("search"),m=n.getProfile("node").dbnode;b.param.transaction={};b.param.limit=e;b.param.search=h;b.param.node=m;b.timestamp.from=g.from.getTime();b.timestamp.to=g.to.getTime();a.forEach(c.transaction,function(a,d){b.param.transaction[a.name]=!0});d.dataLoading=!0;f.searchByMethod(b).then(function(b){d.count=b.length;d.gridOpts.data=b;l(function(){a.element(k).resize()},0)},function(a){})["finally"](function(){d.dataLoading=
!1})},d.processSearchResult(),d.swapData=function(){},d.hashCode=function(a){for(var b=0,d=0;d<a.length;d++)b=a.charCodeAt(d)+((b<<5)-b);return b},d.intToARGB=function(a){return a>>24&255},d.getBkgColorTable=function(a){a=d.hashCode(a);return{"background-color":"hsl("+d.intToARGB(a)+", 75%, 75%)"}},d.showMessage=function(a,b){var d={timestamp:{from:parseInt(a.entity.micro_ts/1E3),to:parseInt(a.entity.micro_ts/1E3)},param:{search:{id:parseInt(a.entity.id),callid:a.entity.callid},location:{node:a.entity.dbnode},
transaction:{call:!1,registration:!1,rest:!1}}};d.param.transaction[a.entity.trans]=!0;m.open({url:"/templates/dialogs/message.html",cls:"homer-modal-content",id:"message"+(""+a.entity.id+"_"+a.entity.trans).hashCode(),divLeft:b.clientX.toString()+"px",divTop:b.clientY.toString()+"px",params:d,onOpen:function(){console.log("modal1 message opened from url "+this.id)},controller:"messageCtrl"})},d.showTransaction=function(b,c){var e=d.gridApi.selection.getSelectedRows(),f=[],g=[];f.push(b.entity.callid);
a.forEach(e,function(a,b){-1==f.indexOf(a.callid)&&f.push(a.callid);-1==f.indexOf(a.callid_aleg)&&1<a.callid_aleg.length&&f.push(a.callid_aleg);-1==g.indexOf(a.dbnode)&&g.push(a.dbnode)});var e={timestamp:{from:parseInt(b.entity.micro_ts/1E3)-300,to:parseInt(b.entity.micro_ts/1E3)+300},param:{search:{id:parseInt(b.entity.id),callid:f},location:{node:g},transaction:{call:!1,registration:!1,rest:!1}}},h=n.getProfile("timerange");e.timestamp.to=h.to.getTime();e.param.transaction[b.entity.trans]=!0;m.open({url:"/templates/dialogs/transaction.html",
cls:"homer-modal-content",id:"trans"+(""+b.entity.callid+"_"+b.entity.dbnode).hashCode(),params:e,divLeft:c.clientX.toString()+"px",divTop:c.clientY.toString()+"px",onOpen:function(){console.log("modal1 opened from url",this.id)},controller:"transactionCtrl"})},d.fileOneUploaded=!0,d.fileTwoUploaded=!1,d.gridOpts={enableColumnResizing:!0,enableSorting:!0,enableRowSelection:!0,enableGridMenu:!0,showGridFooter:!0,noUnselect:!1,multiSelect:!0,modifierKeysToMultiSelect:!1,enablePaging:!0,paginationPageSizes:[25,
50,75],paginationPageSize:25,enableFiltering:!0,rowTemplate:'<div ng-style="row.isSelected && {} || grid.appScope.getBkgColorTable(row.entity.callid)"><div ng-repeat="(colRenderIndex, col) in colContainer.renderedColumns track by col.colDef.name" class="ui-grid-cell" ng-class="{ \'ui-grid-row-header-cell\': col.isRowHeader }" ui-grid-cell></div></div>',filterOptions:{filterText:"",useExternalFilter:!1},columnDefs:[{field:"id",displayName:"Id",width:50},{field:"milli_ts",displayName:"Date",cellFilter:"date:'yyyy-dd-MM HH:mm:ss.sss'",
width:170},{field:"method",displayName:"Method",cellClass:function(a,b,d,c,e){if("INVITE"===a.getCellValue(b,d))return"blue"},cellTemplate:'<div  ng-click="grid.appScope.showMessage(row,$event)" class="ui-grid-cell-contents""><span class="navText">{{COL_FIELD}}</span></div>'},{field:"reply_reason",displayName:"Reason",width:70},{field:"ruri_user",displayName:"RURI user"},{field:"from_user",displayName:"From User"},{field:"callid",displayName:"CallID",width:160,cellTemplate:'<div  ng-click="grid.appScope.showTransaction(row, $event)" class="ui-grid-cell-contents"><span class="navText">{{COL_FIELD}}</span></div>'},
{field:"user_agent",displayName:"User Agent"},{field:"source_ip",displayName:"Source IP"},{field:"source_port",displayName:"SPort",width:50},{field:"destination_ip",displayName:"Dest. IP"},{field:"destination_port",displayName:"DPort",width:50},{field:"source_ip",displayName:"Source IP"},{field:"proto",displayName:"Proto",width:40},{field:"node",displayName:"Node"}]},d.gridOpts.rowIdentity=function(a){return a.id},d.gridOpts.onRegisterApi=function(a){d.gridApi=a})}]).filter("unixts",function(){return function(a){return a?
...
c=f.trustAsHtml(c);a.sipMessage=c;c=[];for(var h in b[0])"msg"!=h&&b[0].hasOwnProperty(h)&&""!=b[0][h]&&c.push(""+h+": <b>"+b[0][h].split("<").join("&lt;")+"</b><br>");c.push();a.sipDetails="<div id='"+b[0].id+"_details'>"+c.join("")+"</div>";a.trustedHtmlDetails=f.trustAsHtml(a.sipDetails)},function(a){console.log("RZ");console.log(a)})["finally"](function(){a.dataLoading=!1});g(function(){-1!==e.getOpenedModals().indexOf("tempModal")&&e.close("tempModal","var a","var b")},5E3)}]).controller("transactionCtrl",
["$scope",b.modules.core.services.search,"$homerModal","$homerCflow","$timeout","$homerModalParams","$sce",function(b,c,e,g,h,f,l){var k=f.params;b.data=k;b.dataLoading=!0;b.showSipMessage=!0;b.showSipDetails=!1;b.clickSipDetails=function(){console.log("details")};b.id=f.id;b.transaction=[];b.clickArea=[];b.msgCallId=f.params.param.search.callid[0];b.collapsed=[];b.enableQualityReport=!1;b.enableRTCPReport=!1;b.enableLogReport=!1;b.enableXRTPReport=!1;b.exportCanvas=function(){var c=a.element(document.querySelectorAll("#"+
f.id)),c=$(c).find("#cflowcanv"),e=document.createElement("a");e.download="callflow_"+b.msgCallId+".png";e.href=c[0].toDataURL("image/png");e.click()};b.exportPCAP=function(){m(this.data,!1,b.msgCallId)};b.exportTEXT=function(){m(this.data,!0,b.msgCallId)};b.exportShare=function(){b.sharelink="";c.createShareLink(k).then(function(a){a&&(b.sharelink=a[0])},function(a){})["finally"](function(){})};b.toggleTree=function(a){b.collapsed[a]=!b.collapsed[a]};b.drawCanvas=function(a,c){b.clickArea=g.setContext(a,
c)};b.showMessage=function(a,b){var c={timestamp:{from:parseInt(a.micro_ts/1E3),to:parseInt(a.micro_ts/1E3)},param:{search:{id:parseInt(a.id),callid:a.callid},location:{node:a.dbnode},transaction:{call:!1,registration:!1,rest:!1}}};c.param.transaction[a.trans]=!0;e.open({url:"/templates/dialogs/message.html",cls:"homer-modal-content",id:"message"+(""+a.id+"_"+a.trans).hashCode(),divLeft:b.clientX.toString()+"px",divTop:b.clientY.toString()+"px",params:c,onOpen:function(){console.log("modal1 message opened from url "+
this.id)},controller:"messageCtrl"})};b.checkMousePosition=function(c){var e=!1,f=c.offsetX,g=c.offsetY;a.forEach(b.clickArea,function(a){a.x1<f&&a.x2>f&&a.y1<g&&a.y2>g&&(e=!0)});return e};b.clickMousePosition=function(c){var e=!1,f={},g=c.offsetX,h=c.offsetY;a.forEach(b.clickArea,function(a){a.x1<g&&a.x2>g&&a.y1<h&&a.y2>h&&(e=!0,f=a)});e&&("host"==f.type?console.log("clicked on host"):"message"==f.type&&b.showMessage(f.data,c));return e};b.reDrawCanvas=function(){b.drawCanvas(b.id,b.transaction)};
c.searchTransaction(k).then(function(a){a&&(b.transaction=a,b.drawCanvas(f.id,a),0<a.rtpinfo.length&&(b.enableXRTPReport=!0,b.xrtpreport=a.rtpinfo))},function(a){})["finally"](function(){b.dataLoading=!1});b.showRTCPReport=function(a){c.searchRTCPReport(a).then(function(a){0<a.length&&(console.log("RTCP",a),b.enableRTCPReport=!0,b.rtcpreport=a)},function(a){})["finally"](function(){b.dataLoading=!1})};b.showLogReport=function(a){c.searchLogReport(a).then(function(a){0<a.length&&(b.enableLogReport=
!0,b.logreport=a)},function(a){})["finally"](function(){b.dataLoading=!1})};b.showQualityReport=function(a){c.searchQualityReport("short",a).then(function(a){0<a.length&&(b.enableQualityReport=!0,b.qualityreport=a)},function(a){})["finally"](function(){b.dataLoading=!1})};b.showRTCPReport(k);b.showLogReport(k);b.showQualityReport(k);console.log("Reporting...",k);var m=function(a,b,d){c.makePcapTextforTransaction(a,b).then(function(a){var c="HOMER5_"+d+".pcap",e="application/pcap";b&&(c="HOMER5_"+
d+".txt",e="attacment/text;charset=utf-8");a=new Blob([a],{type:e});saveAs(a,c)},function(a){})["finally"](function(){})};h(function(){-1!==e.getOpenedModals().indexOf("tempModal")&&e.close("tempModal","var a","var b")},5E3);b.treedata2=[{id:1,title:"node1",nodes:[{id:11,title:"node1.1",nodes:[{id:111,title:"node1.1.1",nodes:[]}]},{id:12,title:"node1.2",nodes:[]}]},{id:2,title:"node2",nodes:[{id:21,title:"node2.1",nodes:[]},{id:22,title:"node2.2",nodes:[]}]},{id:3,title:"node3",nodes:[{id:31,title:"node3.1",
...
homer);angular.module("homer.widgets.sipcapture",["adf.provider","highcharts-ng"]).value("localApiUrl","api/").value("sipcaptureApiUrl","api/").config(function(a){a.widget("sipcaptureChart",angular.extend({title:"SIPCapture Charts",description:"Display SIPCapture API data as charts",controller:"sipcaptureCtrl"},{templateUrl:"js/widgets/sipcapture/sipcapture.html",reload:!0,resolve:{sipdata:function(a,d,c){if(c.path&&c.query)return d.get(c,c.path,c.query)}},edit:{templateUrl:"js/widgets/sipcapture/edit.html",
controller:"sipcaptureEditController"},refresh:!0}))}).service("sipcaptureService",function(a,b,d,c){return{get:function(e,g,h){var f=a.defer();e=d+g;g={};try{if(g=JSON.parse(h),!g.timestamp)return f.reject(),f.promise}catch(l){return f.reject(),f.promise}h=c.getProfile("timerange");g.timestamp.from=h.from.getTime();g.timestamp.to=h.to.getTime();b.post(e,g).success(function(a){a&&a.status&&(300>a.status?f.resolve(a.data):f.reject(a.data))}).error(function(){f.reject()});return f.promise}}}).controller("sipcaptureCtrl",
function(a,b,d,c){function e(c){var d={};angular.forEach(c,function(a){var c=1E3*a[b.panel.timefield],e=[];angular.forEach(g,function(b){e.push(a[b])});var f=e.join("|"),l=0;angular.forEach(h,function(b){l=parseInt(a[b])});d.hasOwnProperty(f)||(d[f]=[]);d[f].push([c,l])});var e=[];"pie"==b.chart.type.value?(e.push({type:b.chart.type.value,name:a.$parent.model.title,data:[]}),angular.forEach(d,function(a,b){var c=0;angular.forEach(a,function(a){c+=a[1]});e[0].data.push([b,c])})):angular.forEach(d,
function(a,c){var d={};d.name=c;d.type=b.chart.type.value;a.sort(function(a,b){return a[0]-b[0]});d.data=a;e.push(d)});return e}var g=b.panel.fieldname.split(";"),h=b.panel.fieldvalue.split(";");a.reloadIt=function(){a.$parent.changeReloading(!0);c.get(b,b.path,b.query).then(function(b){b=e(b);console.log(b);for(var c=a.chartObj,d=0,f=c.series.length;d<f;d++)c.series[d].update({data:b[d].data},!0);a.$parent.changeReloading(!1)},function(a){console.log("RZ");console.log(a)})};var f=e(d);d&&("pie"==
b.chart.type.value?(a.chartConfig={chart:{type:"pie",plotBackgroundColor:null,plotBorderWidth:null,plotShadow:!1},title:{text:a.$parent.model.title,style:{display:"none"}},plotOptions:{pie:{allowPointSelect:!0,cursor:"pointer",depth:0,dataLabels:{enabled:!0,color:"#000000",connectorColor:"#000000",format:"<b>{point.name}</b>: {point.percentage:.1f} %"},showInLegend:!1}},series:f},b.chart.ccc&&(d=[],d=b.chart.ccc.split(","),a.chartConfig.hasOwnProperty("colors")||(a.chartConfig.colors=[]),a.chartConfig.colors=
d,console.log("CCOLOR-PIE:",a.chartConfig)),b.chart.ddd&&(a.chartConfig.chart.options3d={enabled:!0,alpha:45,beta:0},a.chartConfig.plotOptions.pie.depth=35),b.chart.size&&(a.chartConfig.hasOwnProperty("size")||(a.chartConfig.size={}),0<parseInt(b.chart.size.width)&&(a.chartConfig.size.width=b.chart.size.width),0<parseInt(b.chart.size.height)&&(a.chartConfig.size.height=b.chart.size.height)),b.chart.legend&&b.chart.legend.enabled&&1==b.chart.legend.enabled&&(a.chartConfig.plotOptions.pie.showInLegend=
b.chart.legend.enabled,a.chartConfig.plotOptions.pie.dataLabels.enabled=!1)):(a.chartConfig={chart:{type:b.chart.type.value},title:{text:a.$parent.model.title,style:{display:"none"}},xAxis:{type:"datetime"},yAxis:{title:{text:null},min:0},plotOptions:{column:{}},tooltip:{},legend:{enabled:!1,borderWidth:0},series:f},a.chartConfig.chart.zoomType="x",a.chartConfig.tooltip.crosshairs=!0,a.chartConfig.tooltip.shared=!0,b.chart.ccc&&(d=[],d=b.chart.ccc.split(","),a.chartConfig.colors=d),b.chart.sss&&(d=
b.chart.type.value,a.chartConfig.plotOptions[d]={},a.chartConfig.plotOptions[d].stacking=b.chart.ppp?"percent":"normal"),b.chart.ddd&&(a.chartConfig.chart.options3d={enabled:!0,alpha:15,beta:10,depth:50,viewDistance:30},a.chartConfig.plotOptions.column.depth=25,a.chartConfig.chart.margin=60),b.chart.size&&(a.chartConfig.hasOwnProperty("size")||(a.chartConfig.size={}),0<parseInt(b.chart.size.width)&&(a.chartConfig.size.width=b.chart.size.width),0<parseInt(b.chart.size.height)&&(a.chartConfig.size.height=
b.chart.size.height)),b.chart.legend&&(b.chart.legend.align&&(a.chartConfig.legend.align=b.chart.legend.align),b.chart.legend.enabled&&(a.chartConfig.legend.enabled=b.chart.legend.enabled),b.chart.legend.layout&&(a.chartConfig.legend.layout=b.chart.legend.layout)),b.chart.yaxis&&b.chart.yaxis.title&&0<b.chart.yaxis.title.length&&(a.chartConfig.yAxis.title.text=b.chart.yaxis.title)))}).controller("sipcaptureEditController",function(a,b,d,c){a.charts=[{id:1,label:"Spline",value:"spline"},{id:2,label:"Line",
value:"line"},{id:3,label:"Area spline",value:"areaspline"},{id:4,label:"Bar",value:"bar"},{id:5,label:"Scatter",value:"scatter"},{id:6,label:"Pie",value:"pie"},{id:7,label:"Column",value:"column"},{id:8,label:"Gauge",value:"solidgauge"},{id:9,label:"Heatbox",value:"heatmap"}];a.legend_align=[{name:"center",value:"center"},{name:"right",value:"right"},{name:"left",value:"left"}];a.legend_layout=[{name:"horizontal",value:"horizontal"},{name:"vertical",value:"vertical"}];a.updateDebugUrl=function(){var e=
d+a.config.path;try{var g=JSON.parse(a.config.query),h=c.getProfile("timerange");g.timestamp.from=h.from.getTime();g.timestamp.to=h.to.getTime();a.debug="curl -v --cookie 'HOMERSESSID="+b.HOMERSESSID+"' -X POST \\\n-d '"+JSON.stringify(g)+"' \\\n \""+window.location.protocol+"//"+window.location.host+"/"+e+'"\n';a.parsingStatus="ok";a.parsingColorClass="green"}catch(f){a.parsingStatus="Bad parsing: ["+f.message+"]",a.parsingColorClass="red"}};a.updateDebugUrl()});angular.module("homer.widgets.querycap",["adf.provider","highcharts-ng"]).value("localApiUrl","api/").value("querycapApiUrl","http://homer.yourhost.org:8086").config(function(a){a.widget("querycapChart",angular.extend({title:"QueryCapture Charts",description:"Display QueryCapture API data as charts",controller:"querycapCtrl"},{templateUrl:"js/widgets/querycap/querycap.html",reload:!0,resolve:{sipdata:function(a,d,c){c.query||(c.query='select * from "homer.stats.counters.polycomMos.count" where time > $fromts AND time < $tots;');
...
c.path,c.query)}},edit:{templateUrl:"js/widgets/querycap/edit.html",controller:"querycapEditController"},refresh:!0}))}).service("querycapService",function(a,b,d,c){return{get:function(d,g,h){var f=a.defer(),l=c.getProfile("timerange");h=h.replace(/\$fromts/g,1E3*l.from.getTime()+"u");h=h.replace(/\$tots/g,1E3*l.to.getTime()+"u");g=g+"?u="+d.db.user+"&p="+d.db.password+"&db="+d.db.name+"&q="+encodeURIComponent(h);b.defaults.useXDomain=!0;delete b.defaults.headers.common.Authorization;b.get(g).success(function(a){var b=
...
a.reloadIt=function(){a.$parent.changeReloading(!0);c.get(b,b.path,b.query).then(function(b){b=g(b);for(var c=a.chartObj,d=b.length,e=0,m=c.series.length;e<m;e++)e<d?c.series[e].update(b[e],!0):c.series[e]&&c.series[e].remove(!1);for(e;e<d;e++)c.addSeries(b[e],!0);a.$parent.changeReloading(!1)},function(a){})};e=g(d);d&&("pie"==b.chart.type.value?(a.chartConfig={chart:{type:"pie",plotBackgroundColor:null,plotBorderWidth:null,plotShadow:!1},title:{text:a.$parent.model.title,style:{display:"none"}},
...
"ok";a.parsingColorClass="green"}};a.updateDebugUrl()});angular.module("homer.widgets.elasticap",["adf.provider","highcharts-ng","base64"]).value("localApiUrl","api/").value("elasticapApiUrl","http://homer.sipcapture.org:8086").config(function(a){a.widget("elasticapChart",angular.extend({title:"Elastic Facets",description:"Display Elasticsearch Facets",controller:"elasticapCtrl"},{templateUrl:"js/widgets/elasticap/elasticap.html",reload:!0,resolve:{sipdata:function(a,d,c){if(c.path&&c.query)return d.get(c,c.path,c.query)}},edit:{templateUrl:"js/widgets/elasticap/edit.html",
controller:"elasticapEditController"},refresh:!0}))}).service("elasticapService",function(a,b,d,c,e){return{get:function(d,h,f){var l=a.defer(),k=c.getProfile("timerange");f=f.replace(/"\$fromts"/g,k.from.getTime());f=f.replace(/"\$tots"/g,k.to.getTime());f="undefined"!==typeof d.query_string&&-1<d.query_string.indexOf('"')?f.replace(/"\$query"/g,d.query_string):f.replace(/"\$query"/g,'"'+d.query_string+'"');d.db.user&&-1===h.indexOf("@")&&(console.log("ES: Custom HTTP authentication"),b.defaults.headers.common.Authorization=
"Basic "+e.encode(d.db.password?d.db.user+":"+d.db.password:d.db.user+":"));console.log("ES-URL:",h);f=JSON.parse(f);console.log("ES-QUERY",f);b.post(h,f).success(function(a){d.debugresp=JSON.stringify(a);"histogram"===d.chart.query_type?(console.log("ES: FACETS"),l.resolve(a.facets[0].entries)):"terms"===d.chart.query_type?(console.log("ES: FACET TERMS"),l.resolve(a.facets.terms.terms)):"hits"===d.chart.query_type?(console.log("ES: HITS"),l.resolve(a.hits.hits)):l.reject()}).error(function(){console.log("ES ERROR");
l.reject()});return l.promise}}}).controller("elasticapCtrl",function(a,b,d,c){function e(c){var d={},e=b.panel.fieldname.split(";"),l=b.panel.fieldvalue.split(";");angular.forEach(c,function(a){var c=a[b.panel.timefield],g=[];angular.forEach(e,function(a){g.push(a)});var k=g.join("|"),p=0;angular.forEach(l,function(e){b.panel.fieldsum?(d.hasOwnProperty(k)||(d[k]=[]),p=a[e],d[k][1]=p):(d.hasOwnProperty(k)||(d[k]=[]),p=a[e],d[k].push([c,p]))})});var k=[];"pie"==b.chart.type.value?(k.push({type:b.chart.type.value,
name:a.$parent.model.title,data:[]}),angular.forEach(d,function(a,b){var d=0;angular.forEach(a,function(a){d+=a[1]});k[0].data.push([b,d])})):angular.forEach(d,function(a,d){a.sort(function(a,b){return a[0]-b[0]});k.push({name:d,type:b.chart.type.value,data:a})});console.log("ES-SERIES-DATA:",k);return k}b.panel&&(b.panel.fieldname.split(";"),b.panel.fieldvalue.split(";"),a.reloadIt=function(){a.$parent.changeReloading(!0);c.get(b,b.path,b.query).then(function(b){b=e(b);var d=a.chartObj;if("undefined"!==
typeof d.series){for(var c=0,l=d.series.length;c<l;c++)"undefined"!==typeof d.series[c]&&"undefined"!==typeof b[c]&&d.series[c].update({data:b[c].data},!0);a.$parent.changeReloading(!1)}},function(a){})},d&&(d=e(d),"pie"==b.chart.type.value?(a.chartConfig={chart:{type:"pie",plotBackgroundColor:null,plotBorderWidth:null,plotShadow:!1},title:{text:a.$parent.model.title,style:{display:"none"}},plotOptions:{pie:{allowPointSelect:!0,cursor:"pointer",depth:0,dataLabels:{enabled:!0,color:"#000000",connectorColor:"#000000",
format:"<b>{point.name}</b>: {point.percentage:.1f} %"},showInLegend:!1}},series:d},b.chart.ccc&&(d=[],d=b.chart.ccc.split(","),a.chartConfig.hasOwnProperty("colors")||(a.chartConfig.colors=[]),a.chartConfig.colors=d,console.log("CCOLOR-PIE:",a.chartConfig)),b.chart.ddd&&(a.chartConfig.chart.options3d={enabled:!0,alpha:45,beta:0},a.chartConfig.plotOptions.pie.depth=35),b.chart.size&&(a.chartConfig.hasOwnProperty("size")||(a.chartConfig.size={}),0<parseInt(b.chart.size.width)&&(a.chartConfig.size.width=
...
function(){a.newnode=[];a.activeTab=!1;a.activeNewTab=!0;a.editTab=!0;a.newTab=!1;a.activeMainTab=!1;a.tabShown=!1};a.addNewNode=function(){console.log("NEW USER");a.errorNewNodeShow=!1;a.removeNodeTab();var b={};b.param={host:a.newnode.host,dbname:a.newnode.dbname,dbport:a.newnode.dbport,dbusername:a.newnode.dbusername,dbpassword:a.newnode.dbpassword,dbtables:a.newnode.dbtables,name:a.newnode.name,status:a.newnode.status};c.save(b).then(function(b){console.log("ZZZZZZZZZZZZ");a.gridOptions.data.length=
0;e(function(){a.gridOptions.data=b})})};a.doEditNode=function(){console.log("Edit USER");a.editTab=!1;a.newTab=!1;a.activeNewTab=!1;a.activeMainTab=!0;a.activeTab=!0;a.errorEditNodeShow=!1;a.tabShown=!0;var b={};b.param={host:a.noderec.host,dbname:a.noderec.dbname,dbport:a.noderec.dbport,dbusername:a.noderec.dbusername,dbpassword:a.noderec.dbpassword,dbtables:a.noderec.dbtables,name:a.noderec.name,status:a.noderec.status,id:a.noderec.id};c.update(b).then(function(b){a.gridOptions.data.length=0;e(function(){a.gridOptions.data=
b})})};a.doDeleteNode=function(){console.log("Delete USER");h.swal({title:"Are you sure?",text:"Your will not be able to recover this!",type:"warning",showCancelButton:!0,confirmButtonColor:"#DD6B55",confirmButtonText:"Yes, delete it!",closeOnConfirm:!0,closeOnCancel:!0},function(b){b&&(a.editTab=!1,a.newTab=!1,a.activeNewTab=!1,a.activeMainTab=!0,a.activeTab=!0,a.tabShown=!0,c["delete"](a.node.id).then(function(b){a.gridOptions.data.length=0;e(function(){a.gridOptions.data=b})}),a.removeAlarmTab())})};
a.gridOptions={showFooter:!1,enableSorting:!0,enableFiltering:!0,multiSelect:!1,enableRowSelection:!1,enableSelectAll:!1,enableRowHeaderSelection:!1,rowTemplate:'<div ng-click="grid.appScope.onDblClickRow(row)" ng-dblclick="grid.appScope.onDblClickRow(row)" ng-repeat="(colRenderIndex, col) in colContainer.renderedColumns track by col.colDef.name" class="ui-grid-cell" ng-class="{ \'ui-grid-row-header-cell\': col.isRowHeader }" ui-grid-cell ></div>',noUnselect:!0};a.gridOptions.rowIdentity=function(a){return a.id};
...
d.path("/result")};a.clearSearchForm=function(b){c.profileScope.search={};c.setProfile("search",a.newObject)};a.processSearchResult=function(a){var b={param:{},timestamp:{}},d=c.getProfile("transaction"),e=c.getProfile("limit"),m=c.getProfile("timerange"),n=c.getProfile("search"),q=c.getProfile("node").dbnode;b.param.transaction={};b.param.limit=e;b.param.search=n;b.param.node=q;b.timestamp.from=m.from.getTime();b.timestamp.to=m.to.getTime();angular.forEach(d.transaction,function(a,d){b.param.transaction[a.name]=
...
{name:"msg",selection:"Message"},{name:"diversion",selection:"Diversion"},{name:"via_1",selection:"VIA"},{name:"source_ip",selection:"Source IP"},{name:"destion_ip",selection:"Dest. IP"},{name:"source_port",selection:"Source Port"},{name:"destination_port",selection:"Dest. Port"},{name:"node",selection:"DB Node"},{name:"uniq",selection:"Unique"},{name:"orand",selection:"Logic OR"},{name:"proto",selection:"Protocol"},{name:"family",selection:"Family"},{name:"limit",selection:"Limit Query"},{name:"transaction",
selection:"Transaction"},{name:"dbnode",selection:"DB Node"},{name:"restype",selection:"Result Type"}]}).directive("ngEnter",function(){return function(a,b,d){b.bind("keydown keypress",function(b){13===b.which&&(a.$apply(function(){a.$eval(d.ngEnter)}),b.preventDefault())})}}).directive("fieldDisplay",function(a){return{scope:{fieldDisplay:"=",fieldName:"=",fieldHeaders:"="},templateUrl:"js/widgets/quicksearch/template.html",link:function(a,d,c,e){a.selectedItem={name:a.fieldName,selection:a.fieldDisplay};
...
console.log("ES: AGGS COUNT"):"terms"===d.chart.query_type?console.log("ES: AGGS TERMS"):"sum"===d.chart.query_type?console.log("ES: AGGS SUM"):"sum"===d.chart.query_type?console.log("ES: AGGS SUM"):"avg"===d.chart.query_type?console.log("ES: AGGS SUM"):console.log("REJECT: unsupported type?");l.resolve(a.aggregations.data.buckets)}).error(function(){console.log("ES ERROR");l.reject()});return l.promise}}}).controller("elasticaggsCtrl",function(a,b,d,c){function e(d){var c={},e=b.panel.fieldname.split(";"),
l=b.panel.fieldvalue.split(";");angular.forEach(d,function(a){var d=a[b.panel.timefield],g=[];angular.forEach(e,function(a){g.push(a)});var k=g.join("|"),p=0;angular.forEach(l,function(e){b.panel.fieldsum?(c.hasOwnProperty(k)||(c[k]=[]),p=a[e],c[k][1]=p):(c.hasOwnProperty(k)||(c[k]=[]),p=a[e],c[k].push([d,p]))})});var k=[];"pie"==b.chart.type.value?(k.push({type:b.chart.type.value,name:a.$parent.model.title,data:[]}),angular.forEach(c,function(a,b){var d=0;angular.forEach(a,function(a){d+=a[1]});
k[0].data.push([b,d])})):angular.forEach(c,function(a,d){a.sort(function(a,b){return a[0]-b[0]});k.push({name:d,type:b.chart.type.value,data:a})});console.log("ES-SERIES-DATA:",k);return k}console.log(b);b.panel&&(b.panel.fieldname.split(";"),b.panel.fieldvalue.split(";"),a.reloadIt=function(){a.$parent.changeReloading(!0);c.get(b,b.path,b.query).then(function(b){b=e(b);var d=a.chartObj;if("undefined"!==typeof d.series){for(var c=0,l=d.series.length;c<l;c++)"undefined"!==typeof d.series[c]&&"undefined"!==
...
!a.boolDropDownRefreshMenu};a.searchClass="btn btn-primary";a.showMenu=function(){a.showLeftMenu=!a.showLeftMenu};a.doLogout=function(){a.showLeftMenu=!1;a.dropDownUserMenuClass="";f.path(b.modules.auth.routes.logout)};e.subscribe(b.modules.auth.events.userLoggedIn,function(b,c){a.templateSet||(a.showLeftMenu=!0,a.header="templates/header.html",a.menu="templates/left-panel.html",a.ribbon="templates/ribbon.html",a.footer="templates/footer.html",a.shortcut="templates/shortcut.html",a.templateSet=!0,
""!=g.current.name&&"login"!=g.current.name&&g.go(g.current,{},{reload:!0}))});e.subscribe(b.modules.auth.events.userLoggedOut,function(b,c){a.showLeftMenu=!1;a.templateSet&&(a.header=null,a.menu=null,a.ribbon=null,a.footer=null,a.shortcut=null,a.templateSet=!1)});e.subscribe(b.modules.pages.events.hideLeftMenu,function(b,c){a.boolLeftMenu=!0})}]).controller("HomerDatepickerCtrlTest",["$scope","$rootScope",function(b,c){b.today=function(){b.dt=new Date};b.today();b.clear=function(){b.dt=null};b.toggleMin=
function(){b.minDate=b.minDate?null:(new Date).setFullYear(2013,0,1);b.maxDate=b.maxDate?null:(new Date).setFullYear(2032,0,1)};b.toggleMin();b.open=function(a,c){a.preventDefault();a.stopPropagation();b[c]=!0};b.dateOptions={formatYear:"yy",startingDay:1};b.formats=["yyyy/MM/dd","yyyy-MM-dd","dd.MM.yyyy","shortDate"];b.format=b.formats[1];b.deleteDashboard=function(){a.element("#deleteThisDash").click()}}]).controller("HomerDatepickerCtrl",function(b,c,e,g,h){(new Date).setHours((new Date).getHours()-
2);b.timerange=e.profileScope.timerange;var f;(function(){b.$watch(function(){return e.profileScope.timerange},function(a,c){a!==c&&(b.timerange=a)})})();b.toggleMin=function(){b.minDate=b.minDate?null:(new Date).setFullYear(2013,0,1);b.maxDate=b.maxDate?null:(new Date).setFullYear(2032,0,1)};b.toggleMin();b.dateOptions={formatYear:"yy",startingDay:1};b.formats=["yyyy/MM/dd","yyyy-MM-dd","dd.MM.yyyy","shortDate"];b.format=b.formats[1];b.launch=function(){c.create("/templates/dialogs/timerange.html",
"timerangeDialogCtrl",b.timerange).result.then(function(a){b.timerange=a;e.setProfile("timerange",b.timerange)})};b.last=function(a){a=new Date((new Date).setMinutes((new Date).getMinutes()-a));b.timerange={from:a,to:new Date};e.setProfile("timerange",b.timerange);g.broadcast("globalWidgetReload",1)};b.refresh=function(c){console.log("REFRESH:"+c);c=parseInt(c);1>c?g.broadcast("globalWidgetReload",1):a.isDefined(f)?b.cancelRefresh():(b.activeInterval=!0,f=h(function(){b.timerange.from.setSeconds(b.timerange.from.getSeconds()+
c);b.timerange.to.setSeconds(b.timerange.to.getSeconds()+c);e.setProfile("timerange",b.timerange);g.broadcast("globalWidgetReload",1)},1E3*c))};b.cancelRefresh=function(){a.isDefined(f)&&(h.cancel(f),f=void 0,b.activeInterval=!1)}}).controller("timerangeDialogCtrl",function(b,c,e,g){c.timerange=g;c.hstep=1;c.mstep=15;c.options={hstep:[1,2,3],mstep:[1,5,10,15,25,30]};c.$watch("timerange.from",function(a,e){b.info("Date Changed: "+a);c.opened=!1});c.setDate=function(){a.isDefined(c.timerange.from)||
(c.timerange.from=new Date);a.isDefined(c.timerange.to)||(c.timerange.to=new Date)};c.setDate();c.open=function(a,b){a.preventDefault();a.stopPropagation();c[b]=!0};c.done=function(){e.close(c.timerange)}}).config(function(a){a.useBackdrop(!0);a.useEscClose(!0);a.useCopy(!1);a.setSize("sm")}).controller("newDashboardCtrl",function(b,c,e,g){b.dashboard={name:""};b.cancel=function(){c.dismiss("canceled")};b.hitEnter=function(c){!a.equals(c.keyCode,13)||a.equals(b.dashboard,null)||a.equals(b.dashboard,
"")||b.save()};var h=b.uploader=new g({url:"/api/dashboard/upload"});b.save=function(){0<b.uploader.queue.length?h.uploadAll():b.nameDialog.$valid&&c.close(b.dashboard.name)};h.filters.push({name:"customFilter",fn:function(a,b){return 1>this.queue.length}});h.onCompleteAll=function(){console.info("onCompleteAll");c.close("upload")}})})(angular,homer);(function(a,b){a.module(b.modules.auth.name).run(["$rootScope","$location","$state","$stateParams",b.modules.auth.services.authorization,b.modules.auth.services.authentication,b.modules.core.services.profile,function(a,c,e,g,h,f,l,k){var m=!1,n;a.loggedIn=!1;f.session().then(function(e){e.isAuthenticated&&(0==e.isAuthenticated?c.path(b.modules.auth.routes.login):(a.loggedIn=!0,a.currentUser.name=e.name,l.getAll(),e=c.path(),"/login"!=e&&"/logout"!=e||c.path(b.modules.pages.routes.home)));a.loggedIn=
!0},function(a){console.log(a);console.log("bad result");c.path(b.modules.auth.routes.login)});a.$on("$routeChangeStart",function(a,d){var e;m&&d.originalPath!==b.modules.auth.routes.login?(m=!1,c.path(n).replace()):void 0!==d.access&&(e=h.authorize(d.access.loginRequired,d.access.permissions,d.access.permissionCheckType),e===b.modules.auth.enums.authorised.loginRequired?(m=!0,n=d.originalPath,c.path(b.modules.auth.routes.login)):e===b.modules.auth.enums.authorised.notAuthorised&&c.path(b.modules.auth.routes.notAuthorised).replace())})}])})(angular,
