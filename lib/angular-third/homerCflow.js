...
(function() {
	...
	homerCflow.factory('$homerCflow', ['$rootScope', '$controller', '$location', '$timeout', '$compile', '$sniffer', '$q', '$filter', homer.modules.core.services.profile,	
	        function($rootScope, $controller, $location, $timeout, $compile, $sniffer, $q, $filter, userProfile) {
		...
		var self = {
			...
			setContext: function(id, data) {
				...
				angular.forEach(data['calldata'], function(value) {
					...
					drawdata.Text = $filter('date')(new Date(value.micro_ts/1000), "yyyy-MM-dd HH:mm:ss.sss Z", timezone.offset); 
