...
angular.module('homer.widgets.elasticap', ['adf.provider', 'highcharts-ng','base64'])
  ...
  .config(function(dashboardProvider){
    ...
    var widget = {
      templateUrl: 'js/widgets/elasticap/elasticap.html',
      reload: true,
      resolve: {
        sipdata: function($scope, elasticapService, config){
          if (config.path && config.query){
            return elasticapService.get(config, config.path, config.query);
          }
        }
      },
      edit: {
        templateUrl: 'js/widgets/elasticap/edit.html',
        controller: 'elasticapEditController'
      },
      refresh: true
    };
  ...
  .service('elasticapService', function($q, $http, elasticapApiUrl, userProfile, $base64){
    return {
	...
	var myquery = query;	
        ...
        //var url = path + "?u="+config.db.user+"&p="+config.db.password+"&db="+config.db.name+"&q="+ myquery;
        var url = path;
	...
	var objQuery = JSON.parse(query);
		...
		config.debugresp = JSON.stringify(data);
			// if (typeof data.facets[0] !== 'undefined' && data.facets[0].entries) {
		if (config.chart.query_type === 'histogram') {
			console.log('ES: FACETS');
                	deferred.resolve(data.facets[0].entries);
			// } else if (data.facets.terms.terms) {
		} else if (config.chart.query_type === 'terms') {
			console.log('ES: FACET TERMS');
                	deferred.resolve(data.facets.terms.terms);
			// } else if (typeof data.hits.hits !== 'undefined' && data.hits.hits.length > 0) {
		} else if (config.chart.query_type === 'hits') {
			console.log('ES: HITS');
  ...
  .controller('elasticapCtrl', function($scope, config, sipdata, elasticapService){
  ...
  .controller('elasticapEditController', function($scope, $cookies, elasticapApiUrl, userProfile){
