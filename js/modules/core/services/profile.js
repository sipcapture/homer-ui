    ...
    angular.module(homer.modules.core.name).factory(homer.modules.core.services.profile, [
        ...
        function($q, $http, eventbus, $log) {
            ...
            var getRemoteProfile = function(id) {
                var deferred = $q.defer();
                $http.get('api/v1/profile/store/' + id, {
                        handleStatus: [403, 503]
                    })
                    .success(function(data) {
                        var jsonObj = JSON.parse(value);
            ...
            var deleteRemoteProfile = function(id, data) {
                ...
                var defer = $q.defer();
                $http.delete('api/v1/profile/store/' + id, {
                    handleStatus: [403, 503]
                }).then(
                    /* good response */
                    function(results) {
            ...
            var deleteAllRemoteProfile = function() {
                ...
                var defer = $q.defer();
                $http.delete('api/v1/profile/store/', {
                    handleStatus: [403, 503]
                }).then(
                    /* good response */
                    function(results) {
            ...
            var getAll = function() {
                ...
                $http.get('api/v1/profile/store', {
                    ...
                    .success(function(data) {
                        angular.forEach(data.data, function(value, key) {
                            /* workaround for bad json, handeling the exception to prevent the app breaking */
                            try {
