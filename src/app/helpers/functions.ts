...
export class Functions {
  ...
  static getColorByStatus(status: any) {
    let color = '#000';
    if (status === 1) {
      color = '#ea4b35';
    } else if (status === 2) {
  ...
  static getColorRegistrationByStatus(status: any) {
    let color = '#000';
    if (status === 1) {
      color = '#ea4b35';
    } else if (status === 2) {
  ...
  static getUriParams(): any {
    return window.location.search ? window.location.search.split('&')
      .map((i: any) => i.replace('?', '').split('='))
      .reduce((a: any, b) => (a[b[0]] = b[1], a), {}) : null;
  }
  ...
  static saveToFile(data: any, filename: any, type = 'application/octet-stream') {
  ...
  static console2file(data: any, filename: any) {
    if (!data) {
