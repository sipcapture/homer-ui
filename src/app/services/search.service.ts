...
export class SearchService {
  ...
  constructor(
    ...
    if (params && params.param && params.param.search) {
      ...
      if (params && params.timestamp) {
        ...
        const format = (d: any) => new Date(d).toLocaleString().split(',').map((i: any) => i.replace(/\./g, '/')).join('');
  ...
  public getLocalStorageQuery() {
    ...
    if (localData && localData.fields && localData.fields instanceof Array) {
      localData.fields = localData.fields.filter((i: any) => i.name !== ConstValue.CONTAINER);
    }
    return localData;
  }
  ...
  public setTargetContainer(target: any) {
    this.target = target;
  }
  ...
  getQueryFull(id: any, callid: any, selectedCallId: any) {
    const labels = selectedCallId;
  ...
  public queryBuilder_EXPORT(id: any, callid: any, protocol_id = null) {
    const localData = SearchService.currentQuery;
