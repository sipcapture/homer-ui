...
export class DateTimeRangeService {
    ...
    this._sss.sessionStorage.subscribe((data: UserSettings) => {
      if (data.updateType !== 'proto-search') {
        if (data && data.dateTimeRange.dates) {
          ...
          DateTimeRangeService.dateTimeRangr.dates = data.dateTimeRange.dates.map((i: any) => moment(i));
        }
      }
    });
  }
  ...
  getDatesForQuery(isUnixFormat = false): Timestamp {
    const [ from, to ] = (this.getRangeByLabel(DateTimeRangeService.dateTimeRangr.title) ||
      DateTimeRangeService.dateTimeRangr.dates).map((d:any) => {
        d = d.unix() * 1;
        if (isUnixFormat) {
          d *= 1000;
        }
