...
export class PreferenceIpAliasService {
    ...
    getAll(delayBuffer = 1000 * 60 * 5): Promise<PreferenceIpAlias[]> {
        return new Promise<PreferenceIpAlias[]>((resolve, reject) => {
            if (this.aliasesArray?.length > 0 && delayBuffer !== 0) {
                resolve(this.aliasesArray)
            } else {
                this.httpGetBuffer.get<PreferenceIpAlias[]>(this.url, delayBuffer).toPromise()
                    .then(data => {
                        this.aliasesArray = <PreferenceIpAlias[]>data;
                        resolve(data);
                    }, err => {
                        reject(null);
                    });
    ...
    getAliasFileds() {
        return this.getAll().then((alias: any) =>
