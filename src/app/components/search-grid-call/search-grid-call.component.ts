...
export class SearchGridCallComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  ngOnInit() {
    ...
    if (this.inContainer) {
      ...
      setTimeout(() => { /** <== fixing ExpressionChangedAfterItHasBeenCheckedError */
        ...
        if (params && params.timestamp) {
          ...
          const format = d => new Date(d).toLocaleString().split(',').map((i: any) => i.replace(/\./g, '/')).join('');
    ...
    this._pas.getAll().toPromise().then((result: any) => {
      ...
      if (result && result.data) {
        const _advanced = result.data.find((i: any) => i.category === 'search' && i.param === 'transaction');
  ...
  async initSearchSlider(isImportantClear = false) {
    ...
    const [mappingItem] = mappings.filter((i: any) => i.profile === query_protocol_id && i.hepid === query_hepid * 1);
    ...
    if (!this.inContainer) {
      setTimeout(() => {
        ...
        this.searchSliderConfig.countFieldColumns = this.searchSliderConfig.fields.filter((i: any) => i.value !== '').length;
  ...
  getSearchSlider() {
    return this.searchSliderConfig.fields.filter((i: any) => i.value !== '').length;
  }
  ...
  private async openTransactionByAdvancedSettings() {
    ...
    if (params && params.param && !this.isOpenDialog) {
      ...
      try {
        const setting = advanced.data.filter((i: any) => i.category === 'export' && i.param === 'transaction');
        if (setting && setting[0] && setting[0].data) {
          ...
          if (openwindow === true) {
            ...
            this.openTransactionDialog({
              data: sids.map(j => rowData.filter((i: any) => i.sid === j)[0])[0]
            }, null, sids);
          }
        }
      } catch (err) { }
  ...
  private localStateHeaders(apiColumn) {
    ...
    apiColumn.forEach(col => {
      const f = h.filter((i: any) => i.field === col.field && i.name === col.headerName)[0];
      if (f) {
        col.hide = !f.selected;
      }
    });
    h.forEach(col => {
      const f = apiColumn.filter((i: any) => i.field === col.field && i.headerName === col.name)[0];
      if (f) {
        _apiColumn.push(f);
      }
    });
  ...
  public update(isImportant = false) {
    ...
    if (this.isLokiQuery) {
      this._srs.getData(this.queryBuilderForLoki()).toPromise().then(result => {
        this.loader = false;
        this.rowData = result.data.sort((a: any, b: any) => {
  ...
  public openTransactionDialog(row, mouseEventData = null, callisArray = null) {
    ...
    if ((this.arrWindow.filter((i: any) => i.id === sid)[0] != null)) {
      return;
    }
    ...
    /* clear from clones */
    const selectedCallId = callisArray || selectedRows.map((i: any) => i.sid).reduce((a, b) => {
      if (a.indexOf(b) === -1) {
        a.push(b);
      }
      return a;
    ...
    const timeArray = selectedRows.map((i: any) => i.create_date || i.update_ts);
    ...
    this._cts.getTransaction(request).toPromise().then(res => {
      const allCallIds = res.data.calldata.map((i: any) => i.sid).sort().filter((i, k, a) => a[k - 1] !== i);
  ...
  public async addWindowMessage(row: any, mouseEventData = null) {
    if ((this.arrMessageDetail.filter((i: any) => i.id === row.data.id)[0] != null)) {
      return;
    }
    ...
    if (row.isLog || (row.data.payloadType === 1 && (row.data.raw || row.data.item && row.data.item.raw))) {
      ...
      mData.data.messageDetailTableData = Object.keys(mData.data).map((i: any) => {
        let val;
        if (i === 'create_date') {
      ...
      }).filter((i: any) => typeof i.value !== 'object' && i.name !== 'raw');
  ...
  onColumnMoved(event) {
    const bufferData = Functions.cloneObject(event.api.columnController.gridColumns.map((i: any) => i.colDef));
    let lsIndex = 'result-state';
    if (this.id) {
      lsIndex += `-${this.id}`;
    }
    localStorage.setItem(lsIndex, JSON.stringify(bufferData.map((i: any) => ({
      name: i.headerName,
      field: i.field,
      selected: !i.hide
    }))));
