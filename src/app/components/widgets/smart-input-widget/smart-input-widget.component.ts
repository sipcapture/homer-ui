...
export class SmartInputWidgetComponent implements IWidget, OnInit, AfterViewInit, OnDestroy {
  ...
  @ViewChild('onlySmartFieldElement', { static: false }) onlySmartFieldElement: any;
  ...
  private initSliderSmartInput(onlyFieldsDoParse = false) {
    ...
    if (this.onlySmartFieldElement) {
      ...
      if (onlyFieldsDoParse && configData && Object.keys(configData).length !== 0) {
        ...
        this.onlySmartFieldTEXT = fields.map((item: any) => {
        ...
        const fSmartinput: any = this._fields.find((i: any) => i.field_name === 'smartinput');
        if (fSmartinput && fSmartinput.value && fSmartinput.value !== '') {
          this.onlySmartFieldTEXT = fSmartinput.value;
        } else {
          this.onlySmartFieldTEXT = this._fields.filter((i: any) => i.value !== '')
            .map((item: any) => `${item.name}="${item.value}"`).join(' AND ');
        }
  ...
  private updateButtonState() {
    ...
    if (m && m.fields_mapping) {
      ...
      this.fields.forEach((i: any) => {
        const f: any = m.fields_mapping.find((j: any) => j.id === i.field_name);
        if (f && f.type) {
          i.type = f.type;
        }
        if (f && f.form_type) {
