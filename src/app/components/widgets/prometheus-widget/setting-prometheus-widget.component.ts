...
export class SettingPrometheusWidgetComponent {
  ...
  public prometheusLabel: any;
  public prometheusQuery: any;
  ...
  constructor(
    ...
    try {
      ...
      if (data.panel?.queries) {
        data.dataquery.data.map((v: any, k: any) => ({
  ...
  editRecord(element: any) {
    ...
    this.getMetrics(_prometheusLabels, (data: any) => {
  ...
  getMetrics(arrLabels: any, callback: Function, result = []) {
    if (arrLabels.length > 0) {
      this._ps.getLabels(arrLabels.shift()).subscribe(metrics => {
        metrics = metrics.map((i: any) => {
          let name = i.__name__;
  ...
  updateFilters() {
    ...
    arrStr.forEach((j: any) => {
      this._metricList = Functions.cloneObject(this.metricList.filter((i: any) => i.indexOf(j) !== -1));
    });
  }
  onChartType() {
    this.updateResult();
  ...
  updateCss(tag: string) {
    setTimeout(() => {
      const _selector: any = document.querySelector(`.chips-container-selector.${tag}`);
      const _chipList: any = document.querySelector(`.chips-container.${tag}`);
  ...
  remove(item: any, typeName: string) {
    ...
    const arr = _this[typeName].value;
    const index = arr.indexOf(item);
    this.updateCss(typeName);
    if (index >= 0) {
      arr.splice(index, 1);
      _this[typeName].setValue(arr);
    }
  }
}
