...
export class CodeStyleSmartInputFieldComponent implements OnInit, AfterViewInit {
  ...
  ngAfterViewInit() {
    ...
    this.editor.onfocus = (event: any) => {
      this.isFocusOnField = true;
    };
    this.editor.onfocusout = (event: any) => {
  ...
  async getLabels() {
    try {
      ...
      if (labelsData && labelsData.data && labelsData.data.length > 0) {
        labels = labelsData.data.map((i: any) => i.value);
      }
  ...
  onKeyUpDiv(event: any) {
    if (!!{ Shift: 1, Control: 1, Alt: 1, Backspace: 1 }[event.key as string]) {
      this.updateEditor(null);
      return;
    }
  ...
  private typeInTextarea(str: any, autoClear = false) {
    try {
      const sel: any = window.getSelection();
