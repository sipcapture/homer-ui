...
export class TransactionFilterComponent {
    ...
    set dataItem(data) {
         ...
         * IP filter
         */
        this.checkboxListFilterIP = Functions.arrayUniques(hosts.map(i => i.ip)).map(i => ({
            title: i,
            selected: true
        }));
         ...
         * Alias filter
         */
        this.checkboxListFilterAliases = Functions.arrayUniques(hosts.map(i => i.alias)).map(i => ({
            title: i,
            selected: true
        }));
        ...
        this.checkboxListFilterPayloadType = Object.entries(filters.payload)
            ...
            .map(record => {
                ...
                return {
                    title: typeName,
                    selected: this.getPayloadFromLocalStorage(typeName) ||
        ...
        this.checkboxListFilterCallId = callid.map(i => ({
            title: i,
            selected: true,
            color: Functions.getColorByString(i)
        }));
        ...
        TransactionFilterService.setFilter(this.flowFilters);
        ...
        setTimeout(() => {
            ...
            if (type === 'alias') {
                this.checkboxListFilterAliases?.forEach(alias => {
                    const getArrayIPByAlias = Functions.arrayUniques(this.hosts.filter(i => i.alias === alias.title).map(i => i.ip));
                    getArrayIPByAlias?.forEach(ip => {
                        (this.checkboxListFilterIP?.find(({ title }) => title === ip) || {}).selected = alias.selected;
                    });
                });
                this.cdr.detectChanges();
            }
    ...
    getPayloadFromLocalStorage(type: string = 'RTP') {
        const defaultReturn = type === FlowItemType.SIP || type === FlowItemType.SDP;
        let localFilterState: any = localStorage.getItem(UserConstValue.LOCAL_FILTER_STATE) ||
    ...
    restoreFiltersFromLocalStorage() {
        ...
        if (localFilterState) {
            ...
            try {
                localFilterState = Functions.JSON_parse(localFilterState);
                this.combineType = !this._isSingleIP ? localFilterState.combineType : '1none';
                this.isSimplify = localFilterState.isSimplify;
                this.isSimplifyPort = !this._isSingleIP ? localFilterState.isSimplifyPort : false;
                this.isCombineByAlias = !this._isSingleIP ? localFilterState.isCombineByAlias : false;

                this.flowFilters = this.flowFilters || {};
                this.flowFilters.isSimplify = localFilterState.flowFilters.isSimplify;
                this.flowFilters.isSimplifyPort = !this._isSingleIP ? localFilterState.flowFilters.isSimplifyPort : false;
                this.flowFilters.isCombineByAlias = !this._isSingleIP ? localFilterState.flowFilters.isCombineByAlias : false;
                if (
