...
export class DashboardComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  scrollTop() {
    setTimeout(() => {
      ...
      Array.from(document.querySelectorAll('.widget-item')).forEach((i: any) => i.scrollTop = 0);
    }, 100);
  }
  updateTrigger() {
    setTimeout(() => {
  ...
  removeItem($event: any, item: any) {
    ...
    this.dashboardArray = this.dashboardArray.filter((i: any) => JSON.stringify(i) !== JSON.stringify(item));
  ...
  onChangeWidget({ config, id }) {
    this.dashboardArray[this.dashboardArray.map((i: any) => i.id).indexOf(id)].config = config;
    this.save();
  }
  ...
  private getWidgetItemClass(item: DashboardContentModel): IWidgetMetaData {
    ...
    if (item.strongIndex) {
      wItem = WidgetArray.filter((i: any) => i.strongIndex === item.strongIndex);
    } else {
      wItem = WidgetArray.filter((i: any) => i.indexName === item.name);
    }
    return wItem[0];
  }
